...
export class AIService {
  static async generateBio(data: { name: string; school: string; degree: string; major: string; skills: string[] }) {
  ...
  static async generateRoadmap(role: string) {
    const apiKey = getApiKey();
    if (apiKey) {
      try {
        const genAI = new GoogleGenerativeAI(apiKey);
  ...
  static async optimizeResume(bullets: string) {
    const apiKey = getApiKey();
    if (apiKey) {
      try {
        const genAI = new GoogleGenerativeAI(apiKey);
  ...
  static async gradeInterview(role: string, description: string, answers: string[]) {
    const apiKey = getApiKey();
    if (apiKey) {
      try {
        const genAI = new GoogleGenerativeAI(apiKey);
  ...
  static async generateCoverLetter(company: string, role: string, description: string, tone: string) {
    const apiKey = getApiKey();
    if (apiKey) {
      try {
        const genAI = new GoogleGenerativeAI(apiKey);
  ...
  static async generateChatReply(message: string) {
    const apiKey = getApiKey();
    if (apiKey) {
      try {
        const genAI = new GoogleGenerativeAI(apiKey);
