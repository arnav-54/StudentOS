...
router.post('/reissue', async (req, res) => {
  ...
  try {
    ...
    if (!user || user.email !== email) return res.status(404).json({ message: 'User not found' });
...
router.get(
  '/google/callback',
  passport.authenticate('google', { failureRedirect: 'http://localhost:5173/auth?error=oauth_failed' }),
  (req, res) => {
