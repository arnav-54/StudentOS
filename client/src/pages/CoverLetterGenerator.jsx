...
import { FileText, Sparkles, Download, Copy, CheckCircle, RefreshCw, Building, Loader2, FolderLock } from 'lucide-react';
...
const CoverLetterGenerator = () => {
    ...
    const handleGenerate = async () => {
        ...
        setTimeout(() => {
            ...
            // Dynamic user details replacement
            const userName = user?.name || 'Arnav kumar';
            letter = letter.replace(/Alex Mercer/g, userName);
            letter = letter.replace(/\bAlex\b/g, userName.split(' ')[0]);
            letter = letter.replace(/Massachusetts Institute of Technology/g, 'Newton School of Technology');
            letter = letter.replace(/\bMIT\b/g, 'Newton School of Technology');
    ...
    const handleSaveToLocker = async () => {
        ...
        setSaving(true);
        try {
            // Base64 encode the generated letter to store locally in Document URL field
            const base64Str = btoa(unescape(encodeURIComponent(generatedLetter)));
            await axios.post('http://localhost:5001/api/documents', {
                name: `Cover_Letter_${company.replace(/\s+/g, '_')}.txt`,
                type: 'cover_letter',
                url: `text:${base64Str}`
            });
            setSaveSuccess(true);
            ...
            console.error('Failed to save cover letter to vault:', err);
        } finally {
            setSaving(false);
        }
    };
    const handleDownload = () => {
        ...
        const userName = user?.name || 'Arnav kumar';
        const email = user?.email || 'arnav.kumar@adypu.edu.in';
          ...
          <html>
            ...
            <body>
              <div class="header">
                ...
                <div class="contact">Newton School of Technology | ${email}</div>
              </div>
        ...
        <div className="space-y-5">
          ...
          <div className="p-6 rounded-2xl border border-slate-200/60 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/75 backdrop-blur-xl shadow-sm hover:border-slate-300 dark:hover:border-zinc-700/80 transition-all duration-300 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="e.g. Stripe" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/40 text-sm text-slate-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-brand-primary/50"/>
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1">Position / Role</label>
                <input type="text" value={role} onChange={(e) => setRole(e.target.value)} placeholder="e.g. Software Engineer Intern" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/40 text-sm text-slate-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-brand-primary/50"/>
              </div>
            </div>
            <div>
              ...
              <textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={4} placeholder="Paste the job description here for more tailored results..." className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/40 text-xs text-slate-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-brand-primary/50 font-mono"/>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl border border-slate-200/60 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/75 backdrop-blur-xl shadow-sm hover:border-slate-300 dark:hover:border-zinc-700/80 transition-all duration-300 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {toneOptions.map((tone) => (<button key={tone.id} onClick={() => setSelectedTone(tone.id)} className={`p-3 rounded-xl border text-left transition-all ${selectedTone === tone.id
          ...
          {/* Generate Button */}
          <button onClick={handleGenerate} disabled={isGenerating || !company || !role} className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-brand-primary hover:bg-brand-primary/95 text-white font-medium rounded-xl transition-all disabled:opacity-50 shadow-lg shadow-brand-primary/20">
            {isGenerating ? (<>
        ...
        <div className="p-6 sm:p-8 rounded-2xl border border-slate-200/60 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/75 backdrop-blur-xl shadow-sm min-h-[500px] flex flex-col">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-900 pb-3 mb-4">
            ...
            {generatedLetter && (<div className="flex gap-2">
                <button onClick={handleGenerate} className="flex items-center gap-1 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-zinc-800 text-xs font-semibold text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors">
                  <RefreshCw className="h-3 w-3"/>
                  Regenerate
                </button>
                <button onClick={handleCopy} className="flex items-center gap-1 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-zinc-800 text-xs font-semibold text-slate-600 dark:text-zinc-400 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-colors">
                ...
                <button 
                  ...
                  disabled={saving}
                  className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-xs font-semibold transition-all duration-300 ${
                    saveSuccess 
                      ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' 
                      : 'bg-indigo-500/10 border-indigo-500/20 text-indigo-500 hover:bg-indigo-500/20'
                  }`}
                >
                  {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <FolderLock className="h-3.5 w-3.5"/>}
                  {saveSuccess ? 'Saved!' : 'Save to Vault'}
                </button>
                <button onClick={handleDownload} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-primary/10 border border-brand-primary/20 text-xs font-semibold text-brand-primary hover:bg-brand-primary/20 transition-colors">
          ...
          {isGenerating ? (<div className="flex-1 flex flex-col items-center justify-center space-y-4">
            ...
            </div>) : generatedLetter ? (<div className="flex-1 overflow-y-auto">
              <pre className="text-xs text-slate-700 dark:text-zinc-300 leading-relaxed whitespace-pre-wrap font-sans">
                {generatedLetter}
              </pre>
            </div>) : (<div className="flex-1 flex flex-col items-center justify-center text-center space-y-3 py-12">
