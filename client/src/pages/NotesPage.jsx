...
import { FileText, Plus, Search, Trash2, Clock, BookOpen, Hash, Bold, Italic, List, ListOrdered, Heading1, Heading2, Quote, Code, Save, Link2, CheckSquare, } from 'lucide-react';
...
const NotesPage = () => {
    ...
    return (<div className="flex flex-col h-[calc(100vh-7rem)]">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={createNote} className="flex items-center gap-2 px-4 py-2 bg-brand-primary text-white rounded-xl text-xs font-semibold hover:bg-brand-primary/90 transition-all shadow-sm shadow-brand-primary/20">
      ...
      <div className="flex flex-1 gap-4 min-h-0 overflow-hidden">
        ...
        <div className="w-72 shrink-0 flex flex-col gap-3 overflow-hidden">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search notes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-bg-cardDark text-xs focus:outline-none focus:ring-1 focus:ring-brand-primary/40"/>
          </div>

          {/* Note Cards */}
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {filteredNotes.map((note) => (<motion.button key={note.id} onClick={() => {
                setSelectedId(note.id);
                setIsEditing(false);
            }} whileHover={{ x: 2 }} className={`w-full text-left p-3 rounded-xl border transition-all duration-200 ${selectedId === note.id
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {note.tags.slice(0, 2).map((tag) => (<span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-primary/10 text-brand-primary font-semibold">
                      {tag}
                    </span>))}
                </div>
              </motion.button>))}
          ...
          {selectedNote ? (<>
              ...
              <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-slate-100 dark:border-zinc-900">
                ...
                <div className="flex items-center gap-2">
                  {isEditing ? (<button onClick={saveNote} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-primary text-white rounded-lg text-xs font-semibold hover:bg-brand-primary/90 transition-all">
                      <Save className="h-3.5 w-3.5"/>
                      Save
                    </button>) : (<button onClick={() => startEdit(selectedNote)} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-primary text-white rounded-lg text-xs font-semibold hover:bg-brand-primary/90 transition-all">
                      Edit
                    </button>)}
                  <button onClick={() => deleteNote(selectedNote.id)} className="p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/20 text-slate-400 hover:text-brand-danger transition-colors">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {isEditing ? (<div className="space-y-3">
                    <input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} placeholder="Note title..." className="w-full text-xl font-bold bg-transparent border-0 outline-none text-slate-900 dark:text-white placeholder-slate-300"/>
                    <div className="flex items-center gap-2">
                      <Hash className="h-3.5 w-3.5 text-slate-400"/>
                      <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder="Tags (comma separated)" className="flex-1 text-xs bg-transparent border-0 outline-none text-slate-500 dark:text-zinc-400 placeholder-slate-300"/>
                    </div>
                    <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full min-h-[400px] text-sm bg-transparent border-0 outline-none resize-none text-slate-700 dark:text-zinc-300 font-mono leading-relaxed" placeholder="Write in markdown..."/>
                  </div>) : (<div>
                    <h1 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{selectedNote.title}</h1>
                    <div className="flex items-center gap-2 mb-4">
                      {selectedNote.tags.map((tag) => (<span key={tag} className="text-[10px] px-2 py-0.5 rounded-full bg-brand-primary/10 text-brand-primary font-semibold border border-brand-primary/20">
                          {tag}
                        </span>))}
                    ...
                    <div className="prose prose-sm dark:prose-invert max-w-none text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: formatMarkdown(selectedNote.content) }}/>
                  </div>)}
              </div>
