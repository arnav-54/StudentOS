...
interface Achievement {
  ...
  category: 'career' | 'learning' | 'coding' | 'milestone';
  xp: number;
  unlocked: boolean;
  unlockedDate?: string;
  progress?: number;
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="relative">
            <svg className="w-28 h-28" viewBox="0 0 120 120">
              ...
              <circle
                cx="60" cy="60" r="50" fill="none" stroke="url(#levelGradient)" strokeWidth="8"
                strokeLinecap="round"
                strokeDasharray={`${(xpInCurrentLevel / 500) * 314} 314`}
          ...
          <div className="flex-1 space-y-3 text-center sm:text-left">
            ...
            <div className="h-2.5 w-full max-w-sm bg-slate-100 dark:bg-zinc-900 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(xpInCurrentLevel / 500) * 100}%` }}
              />
            </div>
        ...
        {filtered.map((achievement, idx) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ y: -3 }}
              className={`relative p-5 rounded-2xl border ${rarity.border} ${achievement.unlocked ? 'bg-white dark:bg-zinc-950' : 'bg-slate-50/50 dark:bg-zinc-900/30 opacity-60'} ${rarity.glow} transition-all group`}
            >
              {/* Locked overlay */}
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  ...
                  {achievement.progress !== undefined && achievement.total !== undefined && (
                    <div className="space-y-1 pt-1">
                      ...
                      <div className="h-1.5 w-full bg-slate-100 dark:bg-zinc-900 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${achievement.unlocked ? 'bg-brand-success' : 'bg-brand-primary'}`}
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    {achievement.unlocked && achievement.unlockedDate && (
                      <span className="text-[9px] text-slate-400 font-medium">Earned {achievement.unlockedDate}</span>
                    )}
                  </div>
                </div>
              </div>
