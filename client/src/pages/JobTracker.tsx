...
interface Application {
  ...
  const handleMoveStatus = async (id: string, nextStatus: Application['status']) => {
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 overflow-x-auto pb-4">
        {columns.map((col) => {
          ...
          return (
            <div key={col.id} className="min-w-[200px] flex flex-col space-y-3">
              {/* Column Title */}
                ...
                <AnimatePresence mode="popLayout">
                  {colApps.map((app) => (
                    <motion.div
                      ...
                      className="p-4 rounded-xl border border-slate-200/40 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/75 backdrop-blur-xl flex flex-col justify-between hover:shadow-md hover:border-slate-300 dark:hover:border-zinc-700/80 transition-all duration-300 group"
                    >
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-brand-danger transition-opacity"
                          >
                        ...
                        {app.notes && (
                          <p className="text-[9px] text-slate-400 dark:text-zinc-500 bg-slate-50 dark:bg-zinc-900/60 p-1.5 rounded border border-slate-100/50 dark:border-zinc-900/40 leading-normal line-clamp-2">
                            {app.notes}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-zinc-900 flex justify-between items-center text-[10px] text-slate-400 font-semibold">
                        ...
                        <div className="flex gap-1">
                          {columns.filter(c => c.id !== app.status).map(c => (
                            <button
                              key={c.id}
                              onClick={() => handleMoveStatus(app.id, c.id)}
                              className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-900 text-slate-500 hover:text-brand-primary hover:bg-slate-200 dark:hover:bg-zinc-800 transition-colors"
            ...
            <form onSubmit={handleAddApplication} className="space-y-4 text-xs font-semibold">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Developer Relations Intern"
                  value={role}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Referral details, interview dates..."
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/40 focus:outline-none"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-brand-primary text-white rounded-xl font-bold hover:bg-brand-primary/95 transition-all text-center"
              >
                Create Card
              </button>
            </form>
