...
interface TimelineEvent {
  id: string;
  type: 'project' | 'hackathon' | 'internship' | 'research' | 'leadership' | 'achievement';
  title: string;
  subtitle: string;
  description: string;
  date: string;
  link?: string;
}const TimelinePortfolio: React.FC = () => {
  ...
  const [type, setType] = useState<TimelineEvent['type']>('project');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleShare}
            className="flex items-center gap-1.5 px-4 py-2 border border-slate-200 dark:border-zinc-800 bg-white/50 hover:bg-slate-100 dark:bg-zinc-900/40 dark:hover:bg-zinc-900 text-sm font-semibold rounded-xl text-slate-700 dark:text-zinc-300 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-2 bg-brand-primary text-white rounded-xl text-sm font-semibold hover:bg-brand-primary/95 transition-all shadow-md"
          >
      ...
      <div className="p-6 sm:p-8 rounded-2xl border border-slate-200/60 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/75 backdrop-blur-xl shadow-sm hover:border-slate-300 dark:hover:border-zinc-700/80 transition-all duration-300">
        ...
        <div className="relative border-l border-slate-200 dark:border-zinc-800 pl-8 ml-4 sm:ml-8 py-4 space-y-10">
              ...
              >
                ...
                <div className="p-5 rounded-2xl border border-slate-200/40 dark:border-zinc-800/60 bg-white/75 dark:bg-[#0A0A0C]/60 backdrop-blur-xl hover:border-brand-primary/30 transition-colors group relative flex flex-col justify-between gap-3">
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-brand-danger transition-opacity"
          ...
          {events.length === 0 && (
            <div className="text-center py-12 text-slate-400 text-xs pl-0">
            ...
            <form onSubmit={handleAddEvent} className="space-y-4 text-xs font-semibold">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Detailed description of accomplishments..."
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/40 focus:outline-none"
                />
              </div>
